// First, create a global token prefetcher component that you add to your app root

// Then update your TokenSelectModal to be more efficient
"use client";
...
export const TokenSelectModal: FC = () => {
  ...
  // Get chainId from the current token (assuming both tokens are on same chain)
  ...
  const handleSelect = useCallback(
    (tokenData: TokenData) => {
      ...
      if (selectedToken.address === tokenTwo?.address) {
    ...
    [setTokenOne, setTokenTwo, tokenOne, tokenTwo, closeModalOne]
  );
              ...
              <>
                ...
                <List sx={{ p: 0 }}>
                  {filteredTokens.map((token: TokenData, index) => {
                    ...
                    const isSelected = tokenOne?.address === token.address;
