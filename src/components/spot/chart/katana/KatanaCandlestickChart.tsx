...
import {
  useKatanaOHLCData,
  katanaOHLCUtils,
} from "@/hooks/sushiswap/useKatanaOHLCData";
import { usePriceBackend } from "@/hooks/sushiswap/usePriceBackend";
import { useSpotStore } from "@/store/spotStore"; // Import the store
import {
  createChart,
  IChartApi,
  UTCTimestamp,
...
const KatanaCandlestickChart = () => {
  ...
  // Only show chart for Katana chain
  ...
  // Katana OHLC Data Hook - only enabled for Katana chain
  const {
    data: ohlcData,
    isLoading,
    error,
    ...
    resolution,
    days: 30,
    autoRefresh: isKatanaChain, // Only auto-refresh on Katana
    refreshInterval: 300000,
    // enabled: isKatanaChain && !!tokenAddress, // Only enabled for Katana chain/
  });
  ...
  }, [ohlcData, renderKey, isKatanaChain]); // Add isKatanaChain to dependencies
  ...
  }, [tokenAddress, cleanupChart, chartData, isKatanaChain]);
  ...
  }, [tokenAddress, renderKey, initializeChart, cleanupChart, isKatanaChain]);
  ...
  }, [chartData, chartReady, isKatanaChain]);

  // Force re-initialization when resolution changes
  useEffect(() => {
    if (chartReady && tokenAddress && isKatanaChain) {
      const timer = setTimeout(() => {
        initializeChart();
      }, 200);
      return () => clearTimeout(timer);
    }
  }, [resolution, isKatanaChain]);
  ...
  useEffect(() => {
    const handleFocus = () => {
      if (tokenAddress && !chartReady && isKatanaChain) {
        setRenderKey((prev) => prev + 1);
      }
    };
  ...
  }, [tokenAddress, chartReady, isKatanaChain]);
  ...
  return (
    <div key={renderKey} className="w-full h-full relative p-4">
      ...
      <div className="absolute bottom-3 left-4 text-xs text-gray-400 bg-black/70 px-2 py-1 rounded">
        ...
        {chartData.length} | Key: {renderKey}
