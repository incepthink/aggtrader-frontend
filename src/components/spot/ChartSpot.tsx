...
const ChartSpot = () => {
  ...
  // Create token array for Binance price hook (for internal header)
  const tokensForPrice = useMemo((): PortfolioToken[] => {
    const token = createTokenFromSpotToken(tokenOne);
    return token ? [token] : [];
  }, [tokenOne]);

  // Get Binance prices (for internal header)
  const {
    getTokenPrice,
    getTokenChange24h,
  ...
  // Get current price and 24h change (for internal header)
