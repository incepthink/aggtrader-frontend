...
interface FundingHistoryHeaderProps {
...
export const FundingHistoryHeader: React.FC<FundingHistoryHeaderProps> = ({
  selectedMarket,
  onMarketChange,
  fundingPayments,
}) => {
  ...
  const handleDownloadCSV = () => {
    ...
    // Convert funding payments to CSV rows
    const rows = fundingPayments.map((payment) => {
    ...
    link.setAttribute(
      "download",
      `funding_history_${new Date().toISOString().split("T")[0]}.csv`
    );
    ...
    >
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
        ...
        <Button
          ...
          disabled={!fundingPayments || fundingPayments.length === 0}
          sx={{
            px: 2,
            py: 0.75,
            minWidth: "auto",
