// katanaTokens.ts
// Katana Network Token Address Mapping + helpers
// ✅ Includes getToken(symbol) that returns a clean object for destructuring
...
export const KATANA_TOKEN_MAP: Record<string, TokenInfo> = {
...
/** Get token info by address (case-insensitive). Supports "NATIVE" and zero-address. */
...
/**
 * ✅ getToken(symbol): returns a clean object you can destructure.
 *
 * Usage:
 *   const token = getToken("USDC");
 *   if (token) {
 *     const { image, decimals } = token;
 *   }
 *
 * If you REALLY want your funky destructuring:
 *   const t = getToken("USDC");
 *   // t.image -> use it directly instead of tokenImageUSDC
 */
export function getToken(symbol: string): {
  ...
  // ✅ Normalize symbol (vbUSDC → USDC, vbWBTC → WBTC)
  const normalizedSymbol = symbol.startsWith("vb")
    ? symbol.slice(2)
    : symbol;

  const info = getTokenInfoBySymbol(normalizedSymbol);
