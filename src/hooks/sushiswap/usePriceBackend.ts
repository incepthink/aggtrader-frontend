...
const fetchTokenPriceSushi = async (
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      ...
      throw new Error(
        `Sushi API Error: ${error.response?.status} - ${
...
export function usePriceComparison(
  ...
  return {
    ...
    // Comparison helpers
    tokenOneIsHigher:
      query.data && query.data.tokenOne && query.data.tokenTwo
        ? query.data.tokenOne > query.data.tokenTwo
        : undefined,
    priceDifference:
      query.data && query.data.tokenOne && query.data.tokenTwo
        ? Math.abs(query.data.tokenOne - query.data.tokenTwo)
        : undefined,
...
export function usePriceBackendManual() {
  ...
  const fetchPrice = async (address: Address, chainId: SupportedChain = 1) => {
    try {
      ...
      // Update the cache
      queryClient.setQueryData(["sushiTokenPrice", address, chainId], price);
...
export function useMultiChainPriceComparison(
  ...
  const batchQuery = useBatchPriceBackend(address ? tokenPairs : [], options);
  ...
  return {
    ...
    getPriceOnChain: (chainId: SupportedChain) => {
      return batchQuery.data?.find((result) => result.pair.chainId === chainId)
        ?.data;
    },
