  ...
  const fetchOHLCData = useCallback(async () => {
    ...
    try {
      ...
      if (response.data.status === "success") {
        ...
        throw new Error(
          response.data.msg || "Failed to fetch Katana OHLC data"
        );
      }
    } catch (err: any) {
...
export const katanaOHLCUtils = {
  ...
  isKatanaData: (metadata: KatanaOHLCMetadata): boolean => {
    return (
      metadata.dexId === "katana-sushiswap" &&
      metadata.dataSource === "subgraph"
    );
  },
  ...
  hasGoodLiquidity: (data: OHLCPoint[]): boolean => {
    ...
    const avgVolume =
      katanaOHLCUtils.getTotalVolume(recentVolume) / recentVolume.length;
...
export const KatanaOHLCExample = () => {
  ...
  return (
    <div>
      ...
      <p>
        Current Price: ${katanaOHLCUtils.formatPrice(data.metadata.priceUSD)}
      </p>
